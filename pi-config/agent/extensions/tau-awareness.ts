...
The container is recreated on restarts: only mounted paths (\`~/.pi/agent/\`, \`/workspace/\`, \`~/scripts/\`) and the Docker volumes (\`~/.treehouse\`, \`~/.no-mistakes\`) survive. Details live in skills — read them when the task touches their territory:
...
- **\`tau-git-shipping\`** — how git and GitHub work here and how changes ship (treehouse worktrees, the no-mistakes pipeline, the \`tau_share\` no-git rule). **Read before any git operation.**
